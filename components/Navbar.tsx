...
import { getCartCount } from "../app/lib/cart"; // <- ON CHANGE ICI

export default function Navbar() {
  const [query, setQuery] = useState("");
  const [showSearch, setShowSearch] = useState(false);
  const [openMobile, setOpenMobile] = useState(false);
  const [cartCount, setCartCount] = useState(0); // <- STATE LOCAL
  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      setResults({
        products: allProducts.filter(p => p.name.toLowerCase().includes(query.toLowerCase())).slice(0, 5),
    ...
    <header className="sticky top-0 z-50 w-full border-b border-gray-200 bg-white/90 backdrop-blur-md">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-6">
        <Link href="/" className="flex items-center gap-3">
          <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-blue-700 text-xl font-bold text-white">D</div>
          <div><h1 className="text-lg font-extrabold text-slate-900">DUNAMIS SERVICES GROUP</h1></div>
        </Link>
        ...
        <div className="flex items-center gap-3 relative" ref={searchRef}>
          ...
          <Link href="/panier" className="relative flex items-center gap-1"> {/* <- CORRIGÉ /panier */}
            <ShoppingCart size={22}/>
            {cartCount > 0 && <span className="absolute -right-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-600 text-xs font-bold text-white">{cartCount}</span>}
          </Link>
