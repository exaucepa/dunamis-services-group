...
type Groupage = { /* ... ton type reste identique ... */
  id: number; product_id: string; objectif_participants: number; participants: number;
  prix_groupe: number; date_fin_groupage: string;
...
export default function GroupagePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    const { data, error } = await supabase.from("groupages").select("*, products (id, name, image, price, description)").eq("id", params.id).single();
  ...
  const { data: participation, error } = await supabase.from("participations").insert({
    groupage_id: groupage.id, client_nom: nom, client_telephone: telephone,
    methode_paiement: methode, montant_paye: groupage.prix_groupe, statut: 'en_attente'
  }).select().single();
      ...
      <div className="grid md:grid-cols-2 gap-8 mt-4">
        <div>
          <div className="relative w-full h-96 rounded-2xl overflow-hidden mb-4 bg-gray-100">
            ...
            {groupage.products?.image ? (
              <Image src={groupage.products.image} alt={groupage.products.name} fill className="object-cover"/>
        ...
        <div className="sticky top-4 h-fit p-6 border dark:border-zinc-800 rounded-2xl bg-white dark:bg-zinc-900">
          ...
          <div className="mt-4">
            <div className="flex justify-between text-sm mb-1 font-bold"><span>{groupage.participants} / {groupage.objectif_participants} personnes</span><span>{Math.round(progress)}%</span></div>
